...
class Shopware_Controllers_Frontend_Retargeting extends Enlight_Controller_Action
{
    ...
    public function addDiscountCodeAction()
    {
        ...
        if (isset($params['key']) && isset($params['value']) && isset($params['type']) && isset($params['count'])) {
            if ($params['key'] != '' && $params['key'] == $token && $params['value'] != '' && $params['type'] != '' && $params['count'] != '') {

                require_once(dirname(__FILE__) . '/../../lib/api/Retargeting_REST_API_Client.php');

                $discount_type = (isset($params['type'])) ? (filter_var($params['type'],
                    FILTER_SANITIZE_NUMBER_INT)) : 'Received other than int';
                $discount_value = (isset($params['value'])) ? (filter_var($params['value'],
                    FILTER_SANITIZE_NUMBER_FLOAT)) : 'Received other than float';
                $discount_codes = (isset($params['count'])) ? (filter_var($params['count'],
                    FILTER_SANITIZE_NUMBER_INT)) : 'Received other than int';
                ...
                for ($i = 0; $i < $discount_codes; $i++) {
                    ...
                    $values[] = Shopware()->Db()->quoteInto("(?)", array(
                        $description,
                        $code,
                        $count,
                        $discount_value,
